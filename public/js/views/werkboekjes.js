...
function wbBouwHtml(data) {
  ...
  let nr=0; const stappen = (d.secties||[]).flatMap(sec=>(sec.stappen||[]).map(st=>{ nr++; return `<div class="stap"><div class="stap-nummering"><div class="stap-cirkel">${nr}</div></div><div class="stap-kaart"><h3>${wbEsc(sec.titel||'Stap')} ${nr}</h3><p>${wbEsc(st.stap)}</p>${st.afbeeldingBase64?`<div class="foto-rij een"><div class="foto-vak groot"><img src="${st.afbeeldingBase64}"><div class="foto-label">${wbEsc(st.bijschrift||'Afbeelding bij de stap')}</div></div></div>`:`<div class="foto-rij een"><div class="foto-vak"><span>Foto hier plaatsen</span><div class="foto-label">${wbEsc(st.bijschrift||'Bijschrift')}</div></div></div>`}${st.tip?`<div class="blok tip"><div class="blok-titel">💡 Tip</div>${wbEsc(st.tip)}</div>`:''}</div></div>`;})).join('');
