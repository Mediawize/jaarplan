...
function migreer() {
  ...
  if (!klasCols.includes('roulatieBlokken')) {
    db.exec("ALTER TABLE klassen ADD COLUMN roulatieBlokken TEXT DEFAULT NULL");
    console.log('Migratie: roulatieBlokken kolom toegevoegd aan klassen');
  }
...
const Q = {
  ...
  insKlas: db.prepare('INSERT INTO klassen (id,naam,leerjaar,niveau,vakId,docentId,schooljaar,aantalWeken,urenPerWeek,aantalLeerlingen,docenten,roulatie,roulatieBlok,roulatieStart,roulatieBlokken) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'),
  updKlas: db.prepare('UPDATE klassen SET naam=?,leerjaar=?,niveau=?,vakId=?,docentId=?,schooljaar=?,urenPerWeek=?,aantalLeerlingen=?,docenten=?,roulatie=?,roulatieBlok=?,roulatieStart=?,roulatieBlokken=? WHERE id=?'),
...
module.exports = {
  ...
  getKlas(id) { const k = Q.getKlas.get(id); return k ? { ...k, docenten: parseJSON(k.docenten), roulatieBlokken: parseJSON(k.roulatieBlokken), niveau: k.niveau || '', aantalLeerlingen: k.aantalLeerlingen || 0 } : null; },
  addKlas(d) {
    const id = genId();
    Q.insKlas.run(id, d.naam, d.leerjaar || 1, d.niveau || '', d.vakId || null, d.docentId || null, d.schooljaar || null, d.aantalWeken || 38, d.urenPerWeek || 3, parseInt(d.aantalLeerlingen || 0), JSON.stringify(d.docenten || []), d.roulatie ? 1 : 0, d.roulatieBlok || 5, d.roulatieStart || 35, d.roulatieBlokken ? JSON.stringify(d.roulatieBlokken) : null);
    return this.getKlas(id);
  },
  updateKlas(id, d) {
    const k = this.getKlas(id);
    if (!k) return;
    Q.updKlas.run(d.naam ?? k.naam, d.leerjaar ?? k.leerjaar, d.niveau ?? k.niveau ?? '', d.vakId ?? k.vakId, d.docentId ?? k.docentId, d.schooljaar ?? k.schooljaar, d.urenPerWeek ?? k.urenPerWeek, parseInt(d.aantalLeerlingen ?? k.aantalLeerlingen ?? 0), JSON.stringify(d.docenten ?? k.docenten), d.roulatie !== undefined ? (d.roulatie ? 1 : 0) : k.roulatie, d.roulatieBlok ?? k.roulatieBlok, d.roulatieStart ?? k.roulatieStart, d.roulatieBlokken !== undefined ? (d.roulatieBlokken ? JSON.stringify(d.roulatieBlokken) : null) : k.roulatieBlokken ? JSON.stringify(k.roulatieBlokken) : null, id);
  },
